...
export default function ProductModal({
  ...
  const EMPTY_FORM: ProductForm = { title: "", desc: "", price: "", sizes: [], specs: [] };

// -----------------------------
// ✅ 수정된 상품 불러오기 및 상태 초기화 로직
// -----------------------------
useEffect(() => {
  if (!show) return; // 닫힐 때 굳이 초기화하지 말고(원하면 여기서 해도 됨)
    ...
    (async () => {
      try {
        ...
        if (categoryList && categoryList.length > 0) {
          const savedCategoryId = data.categoryId || data.secondaryCategoryId;
          let foundPId: number | null = null;
          let foundSId: number | null = null;

          for (const pCat of categoryList) {
            const child = pCat.children?.find(c => c.id === Number(savedCategoryId));
            if (child) {
              foundPId = pCat.id;
              foundSId = child.id;
              break;
          ...
          setCategory1(foundPId || Number(savedCategoryId));
          setCategory2(foundSId);
        }
      } catch (err) {
...
}, [show, mode, productId, categoryList]); // categoryList는 여기 포함해도 OK
...
// 서버가 'categoryId'를 기다리고 있다면 이름을 맞춰줍니다.
// 2차 카테고리가 있으면 2차를, 없으면 1차를 보냅니다.
const finalCategoryId = category2 || category1; 
fd.append("categoryId", String(finalCategoryId)); 

// 만약 서버에서 1차, 2차를 따로 다 받는다면 이름을 서버와 똑같이 맞춰야 합니다.
// fd.append("primaryCategoryId", String(category1));
  ...
  onClick={(e) => {
    e.stopPropagation(); // ⬅️ 부모 요소로 클릭 이벤트가 퍼지는 것을 막음
    if (s.stock > 0) {
